...
const
  ...
  resolve = function (_resolve) {
    ...
    return function (filename, source, options) {
      ...
      const [section, group, name] = filename.split('-');

      if (undefined === name) {
        return filename;
      }
      const minimatchString = `${options.basedir}/${section}/?([0-9][0-9]-)${group}/**/${name}`;
      const matches = glob.sync(minimatchString, globOptions);
