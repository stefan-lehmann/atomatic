...
class DevServer {
  ...
  constructor(settings) {
    ...
    const {getSections, getFiles, getCollectedData} = settings.CollectorStore;
    ...
    this.baseDir = this.conf.get('dest');
    this.htmlExt = this.conf.get('htmlExt');
  ...
  addRoutes() {
    const {getSections, getFiles} = this;

    getSections().forEach((section) => {
      this.app.get(section.route, (req, res) => {
        if (getFiles().has(req.url)) {
          const {wrappedBody} = getFiles().get(req.url).render;
          res.send(wrappedBody);
        }
      });
    });
