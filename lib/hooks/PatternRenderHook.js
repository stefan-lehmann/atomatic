...
class PatternRenderHook {
  ...
  replace() {
    const {constructor, replaceObjects, html, replaceReplaceObject} = this;

    this.combinations = replaceObjects.reduce(replaceReplaceObject, [{html}]);
    sgUtil.dumpData(`.tmp/${constructor.name}/combinations-${this.count++}.json`, this.combinations);
    return this;
  }
  ...
  group() {
    ...
    sgUtil.dumpData(`.tmp/${constructor.name}/group.json`, data);
  ...
  replaceReplaceObject(data, replaceObject) {
    const
      {search, versions = [], versionKey} = replaceObject,
      newData = [];
