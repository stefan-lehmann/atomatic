...
class Watcher {
  ...
  onAddFiles(filename, section) {
    ...
    if (minimatch(filename, `**/*.${this.templateExt}`)) {
      ...
      this.emitDelayed(filename, 'onadd');
    }

    if (minimatch(filename, `**/*.${this.dataExt}`)) {
  ...
  onChangeFiles(filename, section) {
    ...
    this.emitDelayed(filename, 'onchange');
  }
  ...
  onUnlinkFiles(filename, section) {
    ...
    if (minimatch(filename, `**/*.${this.templateExt}`)) {
      ...
      this.removeCaches(filename, section);
      this.emitDelayed(filename, 'onunlink');
    }
  ...
  emitDelayed(filename, origin) {
    ...
    if ([htmlExt, templateExt, dataExt].indexOf(extension) !== -1) {
      ...
      this.emitTimeout = setTimeout(() => {
        browserSync.sockets.emit('atomatic:reload');
  ...
  removeCaches(filename, section) {
    ...
    if (section.dataLoader && typeof section.dataLoader.resetCache === 'function') {
      section.dataLoader.resetCache();
    }
