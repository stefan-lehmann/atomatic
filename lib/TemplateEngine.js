...
class TemplateEngine {
  ...
  render(file, global = {}, wrap = true) {

    const
      {filename, componentName, instance, timestamp, saveHtml, saveLocals, renderHook, extension} = file,
      data = extend({}, file.data),
      hash = file.hash ? file.hash : objectHash.MD5({componentName, data, global});
    ...
    if (!this.cache.has(hash) || this.cache.get(hash).timestamp !== timestamp) {
      ...
      source = this.engines.get(extension).render(filename, locals, extend({}, {atomatic: data}, global));
      ...
      if (wrap) {
        html = this.wrap(source, data, filename, instance);
      }
