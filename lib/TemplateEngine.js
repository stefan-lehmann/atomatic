...
class TemplateEngine {
  ...
  render(file) {
    ...
    const
      ...
      defaultWrapperTemplateFilename = path.join(__dirname, `../app/templates/wrapper/${instance}Template.pug`),
      wrapperTemplateFilename = ConfigManager.get(`app.wrapperTemplates.${instance}`, defaultWrapperTemplateFilename);

    let {locals} = data;
    delete data.locals;

    let source = engine.render(filename, locals, {atomatic: data});

    if (typeof renderHook === 'function') {
      ({source, locals} = renderHook(file, source));
    }

    const html = fs.existsSync(wrapperTemplateFilename) ? this.wrap(source, data, filename, wrapperTemplateFilename) : source;
