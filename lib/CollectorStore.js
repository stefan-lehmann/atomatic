...
class CollectorStore {
  ...
  constructor({conf, TemplateEngine}) {
    ...
    this.markFileAsDirty = this.markFileAsDirty.bind(this);
  ...
  getCollectedData() {
    const
      {getSections} = this,
      data = getSections(true).map(section => ({
        collector: section.collector,
        route: section.route,
        title: section.title,
        url: section.url,
        files: this.getSectionFilesData(section)
      }));
  ...
  getSectionFilesData(section) {
    const {getFiles, url} = section;

    return getFiles().map(file => {

      const {filename} = file;

      if (this.dirty.has(path.dirname(filename))) {

        this.cachedFiles.set(filename, {
          title: file.title,
          collector: file.collector,
          orderValue: file.orderValue || path.relative(url, path.dirname(file.url)),
          url: file.url,
          path: path.relative(url, path.dirname(file.url)),
          asyncContentUrls: {
  ...
  markFileAsDirty(filename) {
    this.dirty.set(path.dirname(filename), true);
  }
