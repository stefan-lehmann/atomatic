...
class CollectorStore {
  ...
  constructor(settings) {
    ...
    this.unsetParentFile = this.unsetParentFile.bind(this);
  ...
  unsetFile(filename) {
    const
      ...
      {parentUrl} = this.files.get(url);

    this.urls.delete(base);
    this.files.delete(url);

    this.unsetParentFile(parentUrl);
  }

  unsetParentFile(url = '') {

    if (url === '' || !this.files.has(url)) return;

    const hasCildren = [...this.files].some((file) => file.parentUrl === url);

    if (hasCildren === false) {
      const {filename} = this.files.get(url);
