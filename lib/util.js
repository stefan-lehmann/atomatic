const
  ...
  getTitleFromFilename = function (filePath) {
    return capitalizeString(path.basename(filePath, getFileExtension(filePath)))
      .replace(/\d{2,}|-|_/g, ' ')
      .trim();
  },

  getTitleFromFoldername = function (filePath) {
    return capitalizeString(path.basename(path.dirname(filePath)))
      .replace(/\d{2,}-|_/g, ' ')
      .trim();
  },
  ...
  readRelatedData = function (_filePath, isRequired = true) {
    ...
    if (!fs.existsSync(filePath)) {
      if (isRequired) {
        log(`sgUtil.readRelatedData: resolved '${_filePath}' to '${filePath}' but unable to load`, 'warn');
      }
      return {};
    }
  ...
  readRelatedMarkdown = function (_filePath, isRequired = false) {
  ...
  readJson5File = function (filePath) {
    if (!path.isAbsolute(filePath)) {
      filePath = path.join(ConfigManager.get('rootPath'), filePath);
    }
  ...
  readMarkdownFile = function (filePath, debug) {
