const
  ...
  removeFileExtension = (filePath, onlyLastExt = false) => {
    const i = onlyLastExt ? filePath.lastIndexOf('.') : filePath.indexOf('.');
    return (i < 0) ? filePath : filePath.substr(0, i);
  },

  replaceFileExtension = (filePath, extension, onlyLastExt = false) => {
    const basePath = removeFileExtension(filePath, onlyLastExt);
